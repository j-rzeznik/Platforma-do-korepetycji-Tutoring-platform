...
std::string Przedmiot::getInfo() {
    return "Przedmiot: "+getNazwaPrzedmiotu()+", cena za godzinę: "+std::to_string(static_cast<int>(round(getCenaZaGodzine())))+", cena za materiały: "+std::to_string(getCenaZaMaterialyDydaktyczne())+", liczba nauczycieli: "+std::to_string(getLiczbaNauczycieli());
}
